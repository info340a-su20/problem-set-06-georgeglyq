...
function renderTrack(songTrack) {
  ...
  let recordNewElem = document.createElement("img");
  recordNewElem.src = songTrack.artworkUrl100;
  recordNewElem.alt = songTrack.trackName;
  recordNewElem.title = songTrack.trackName;
  records.appendChild(recordNewElem);
}
...
function fetchTrackList(searchTerm) {
  toggleSpinner();
  let linkHead = URL_TEMPLATE.substring(0, 58);
  let linkUrl = linkHead + searchTerm;
  let promise = fetch(linkUrl) 
